...
# ── Attendee helpers (joint scheduling) ──────────────────────────────────────
...
def _gcal_event_to_session(event: dict) -> Optional[dict]:
    """Convert a raw Google Calendar event dict to the session format used internally."""
...
def _fetch_attendee_sessions(tokens: dict) -> List[dict]:
    ...
    try:
        ...
        # Match calendar service: ~6 months back + forward for speed and consistency.
...
async def _load_attendee_profile(email: str):
    """Load a UserProfile for the attendee from Firestore (bandit state only; default prefs)."""
...
async def search_users(q: str = "", exclude: str = ""):
    """
    Return up to 5 matching CalCoach users.
    - q="": returns recently active users (have a bandit state, sorted by created_at desc)
    - q="<prefix>": returns users whose email starts with that prefix
    exclude is the requesting user's email so they don't appear in their own results.
    """
    q = q.strip().lower()
    exclude = exclude.strip().lower()
    db = get_db()
    results = []

    if not q:
        # Show recently active users (those with a bandit_state_json — they've been onboarded)
        async for doc in (
            ...
            if email and email != exclude and data.get("bandit_state_json"):
                results.append({"email": email, "displayName": data.get("displayName", "")})
            if len(results) >= 5:
                break
    else:
        async for doc in (
